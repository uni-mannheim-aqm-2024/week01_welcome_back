...
vector <- round(runif(n, 0, 100))
matrix <- matrix(round(runif(n*k, 0, 100)),
                 nrow = n,
                 ncol = k)

# write the function
matrix_function <- function(X, y){
  solve(t(X) %*% X) %*% t(X) %*% y
}

matrix_function(X = matrix, y = vector)
```
